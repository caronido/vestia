...
<div class="flex flex-col items-center px-4 py-8 gap-6">
	<h1 class="text-xl font-semibold">Mi Closet</h1>

	{#if outfits.length === 0}
		<div class="text-center py-16 space-y-3">
			<p class="text-4xl">👗</p>
			<p class="text-gray-400 text-sm">Aún no tienes outfits guardados.</p>
			<a
				href="/"
			...
			{#each outfits as outfit (outfit.id)}
				<div class="relative rounded-2xl overflow-hidden shadow-md bg-gray-50">
					<button onclick={() => (fullViewUrl = outfit.imageUrl)} class="w-full cursor-pointer bg-transparent border-0 p-0">
						<img src={outfit.imageUrl} alt="Outfit guardado" class="w-full aspect-square object-cover" loading="lazy" />
					</button>
					<button
